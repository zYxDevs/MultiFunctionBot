...
async def hungama(url):
    ...
    resp2 = client.post('https://www.hungama.com/index.php', params=params, headers=headers, data=data)
    ...
    if metadata['actor_list'] != '':
        rslt += f"<b>Actors List:</b> <i>{bytes(metadata['actor_list'], 'UTF-8')}</i><br>"
