...
async def headfone(url):
    ...
    resp = client.get(url)
    raw_json = re.search("tracks = (.+);", resp.text)[1]
    name = re.search("<div class=channel-info-title> (.+?) </div>", resp.text)[1]
    rslt += f"<i>Channel Name :</i> <b>{name}</b><br>"
    json_data = json.loads(raw_json)
    cdn = re.search("https://(.+?).cloudfront.net", json_data[0]["url"])[1]
    for num, audio in enumerate(json_data, start=1):
...
async def hungama(url):
    ...
    rslt = f"<b>User URL :</b> <code>{url}</code><br>"
    client = requests.Session()
    id = idRegx.search(url)[1]
    resp = requests.get(f"https://www.hungama.com/video/_/{id}/")
    metadata = metadataRegx.search(resp.text)[1][:-1]
